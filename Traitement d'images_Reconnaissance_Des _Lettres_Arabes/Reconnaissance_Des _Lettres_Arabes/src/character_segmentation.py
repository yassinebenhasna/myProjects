...
def baseline_detection(word_img):
    '''Obtenir l’index de base d’un mot donné '''

    HP = projection(word_img, 'horizontal')
    peak = np.amax(HP)

    # Tableau d'indices de l'élément max
    baseline_idx = np.where(HP == peak)[0]

    # Obtenir le premier ou le dernier index
...
def cut_points(word_img, VP, MFV, MTI, baseline_idx):
    ...
    # pour connaître le début du mot
    f = 0
    ...
    while i >= 0:
        ...
        if f == 1:
            ...
            # Obtenez le début et la fin de la région de séparation
